...
(defn- request-workflow
  ...
  (try
    (let [url (str (env :c3integration-url) "&area=WORKFLOW" "&type=" type "&action=" action "&fromuser=" from "&caseid=" case-id
...
(defn- request-workflows
  []
  (doseq [{:keys [type action to from client_id relative_id case_id workflow_id]} (db/get-workflows-to-request {:retry-max (env :c3integration-retry)})]
